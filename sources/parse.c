...
/*   Updated: 2024/07/05 17:47:16 by thfranco         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_tree_node	*parse_expression(t_token **data)
{
    ...
    t_tree_node *left = create_tree_node((*data)->token, (*data)->value);
    if (!left)
    {
    ...
    *data = (*data)->next;
    ...
    {
        t_tree_node *root = create_tree_node((*data)->token, (*data)->value);
        if (!root)
        {
            perror("Failed to create tree node");
            exit(EXIT_FAILURE);
        ...
        root->left = left;
        *data = (*data)->next;
        root->right = parse_expression(data);
        return root;
    }
