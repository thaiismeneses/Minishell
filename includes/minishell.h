...
void			handle_command(t_token **new, char *cmd,
					int flag, t_token **tmp);
...
void			setup_child(int fd_in, int fd_out,
					t_tree_node *node, t_main *main);
...
//expansion.c
int				subst_env_var(t_token *node, int start,
...
int				process_redirect(char *value, int i,
					t_redirect_info *redir_info);
